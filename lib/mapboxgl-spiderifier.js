function MapboxglSpiderfier(map, userOptions) {
  var util = {
    ...
    options = {
      ...
      speed: userOptions.speed || (userOptions.animate ? 200 : 0) // animation speed in milliseconds
    },
    twoPi = Math.PI * 2,
    ...
    if (options.animate) {
      setTimeout(function () {
        util.each(markerObjects.reverse(), function (markerObject, index) {
          ...
          markerObject.elements.parent.style['transitionDelay'] = ((options.speed / 1000) / markerObjects.length * index) + 's';
        });
      });
    }
  ...
  function unspiderfy() {
    util.each(previousMarkerObjects.reverse(), function (oldMarkerObject, index) {
      if (options.animate) {
        oldMarkerObject.elements.parent.style['transitionDelay'] = ((options.speed / 1000) / previousMarkerObjects.length * index) + 's';
        oldMarkerObject.elements.parent.className += ' exit';
      }

      setTimeout(function () {
        oldMarkerObject.mapboxMarker.remove();
      }, options.speed + 100); //Wait for 100ms more before clearing the DOM
    });
    previousMarkerObjects = [];
  }
