function MapboxglSpiderfier(map, userOptions) {
    ...
    if (options.animate || true) {
      setTimeout(function () {
        util.each(markerObjects.reverse(), function (markerObject, index) {
          ...
          markerObject.elements.parent.style['transitionDelay'] = (0.2 / markerObjects.length * index) + 's';
        });
      }, 100);
    }
  ...
  function unspiderfy() {
    var animateSpeed = (options.animate || true) ? 200 : 0;

    util.each(previousMarkerObjects, function (oldMarkerObject, index) {
      if (options.animate || true) {
        $(oldMarkerObject.elements.parent).addClass('initial');
      }
